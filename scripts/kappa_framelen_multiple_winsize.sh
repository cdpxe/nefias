...
#
# kappa_framelen.sh: calculate Kappa compressibility score based on frame
# length of a flows's packets for different window sizes.
...
for windowsize in $WINDOWSIZES; do
	for flow in $FLOWS; do
		...
		cat ${TMPPKTSFILE} | grep $flow | head -1001 | awk -F\, ${FLOWFIELDS} -vwinsize=${windowsize} \
