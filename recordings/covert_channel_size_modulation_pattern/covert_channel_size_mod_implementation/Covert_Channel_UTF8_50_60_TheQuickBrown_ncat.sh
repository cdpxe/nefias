...
#Der Channel sendet Pakete der Größen 50 Byte (0-Bit) und 60 Byte (1-Bit) mittels ncat.
...
while true; do
	...
	for i in "${kod[@]}"; do
		...
		if [ $i = "0" ]
		then
			cat 0Bit_50ByteSize.txt
		else
			cat 1Bit_60ByteSize.txt
		fi
		
		# Schlafe 200 ms. Ansonsten werden auf Empfangsseite die Pakete nicht schnell genug verarbeitet.
		sleep 0.2
